...
#elif defined(CONFIG_MTK_M4U)
m4u_callback_ret_t cmdq_TranslationFault_callback(int port, unsigned int mva,
						  void *data)
{
	char dispatchModel[MDP_DISPATCH_KEY_STR_LEN] = "MDP";
...
void cmdq_mdp_init_module_base_VA(void)
{
	...
	gCmdqMdpModuleBaseVA.MDP_RDMA0 =
	    cmdq_dev_alloc_module_base_VA_by_name("mediatek,mdp_rdma0");
	gCmdqMdpModuleBaseVA.MDP_RDMA1 =
	    cmdq_dev_alloc_module_base_VA_by_name("mediatek,mdp_rdma1");
	gCmdqMdpModuleBaseVA.MDP_RSZ0 =
	    cmdq_dev_alloc_module_base_VA_by_name("mediatek,mdp_rsz0");
	gCmdqMdpModuleBaseVA.MDP_RSZ1 =
	    cmdq_dev_alloc_module_base_VA_by_name("mediatek,mdp_rsz1");
	gCmdqMdpModuleBaseVA.MDP_RSZ2 =
	    cmdq_dev_alloc_module_base_VA_by_name("mediatek,mdp_rsz2");
	gCmdqMdpModuleBaseVA.MDP_WDMA =
	    cmdq_dev_alloc_module_base_VA_by_name("mediatek,mdp_wdma");
	gCmdqMdpModuleBaseVA.MDP_WROT0 =
	    cmdq_dev_alloc_module_base_VA_by_name("mediatek,mdp_wrot0");
	gCmdqMdpModuleBaseVA.MDP_WROT1 =
	    cmdq_dev_alloc_module_base_VA_by_name("mediatek,mdp_wrot1");
	gCmdqMdpModuleBaseVA.MDP_TDSHP =
	    cmdq_dev_alloc_module_base_VA_by_name("mediatek,mdp_tdshp");
	gCmdqMdpModuleBaseVA.MDP_COLOR =
	    cmdq_dev_alloc_module_base_VA_by_name("mediatek,mdp_color");
	gCmdqMdpModuleBaseVA.VENC =
	    cmdq_dev_alloc_module_base_VA_by_name("mediatek,venc");
#endif
}
	...
	if (logLevel >= 1) {
		...
		for (inner = 0; inner < ARRAY_SIZE(bases); ++inner) {
			if (engineFlag & (1LL << bases[inner].engine)) {
				...
				print_hex_dump(
				    KERN_ERR, "", DUMP_PREFIX_ADDRESS, 32, 4,
...
int32_t cmdqMdpResetEng(uint64_t engineFlag)
{
	...
	if (engineToResetAgain != 0) {
		...
		CMDQ_ERR("Reset failed MDP engines(0x%llx), reset again with "
			 "MMSYS_SW0_RST_B\n",
			 engineToResetAgain);
...
void cmdqMdpInitialSetting(void)
{
	...
	mtk_iommu_register_fault_callback(
	    M4U_PORT_MDP_RDMA0, cmdq_TranslationFault_callback, (void *)data);
	mtk_iommu_register_fault_callback(
	    M4U_PORT_MDP_RDMA1, cmdq_TranslationFault_callback, (void *)data);
	mtk_iommu_register_fault_callback(
	    M4U_PORT_MDP_WDMA0, cmdq_TranslationFault_callback, (void *)data);
	mtk_iommu_register_fault_callback(
	    M4U_PORT_MDP_WROT0, cmdq_TranslationFault_callback, (void *)data);
	mtk_iommu_register_fault_callback(
	    M4U_PORT_MDP_WROT1, cmdq_TranslationFault_callback, (void *)data);
#elif defined(CONFIG_MTK_M4U)
	char *data = kzalloc(MDP_DISPATCH_KEY_STR_LEN, GFP_KERNEL);
...
uint32_t cmdq_mdp_rdma_get_reg_offset_src_addr(void) { return 0xF00; }

uint32_t cmdq_mdp_wrot_get_reg_offset_dst_addr(void) { return 0xF00; }

uint32_t cmdq_mdp_wdma_get_reg_offset_dst_addr(void) { return 0xF00; }
...
void testcase_clkmgr_mdp(void)
{
	...
	testcase_clkmgr_impl(
	    ...
	    MDP_WDMA_BASE + cmdq_mdp_wdma_get_reg_offset_dst_addr(), 0xAACCBBDD,
	...
	testcase_clkmgr_impl(CMDQ_ENG_MDP_COLOR0, "CMDQ_TEST_MDP_COLOR",
			     MDP_COLOR_BASE + 0x438, 0x000001AB,
			     MDP_COLOR_BASE + 0x438, true);
#endif
}
...
const char *cmdq_mdp_dispatch(uint64_t engineFlag)
{
	...
	status = cmdq_core_get_running_task_by_engine_unlock(
	    engineFlag, debug_str_len, &task);
	if (status < 0) {
		CMDQ_ERR("Failed: get task by engine flag: 0x%016llx, task "
			 "flag: 0x%016llx\n",
	...
	if ((engineFlag & (1LL << CMDQ_ENG_MDP_RDMA0)) ||
	    (engineFlag & (1LL << CMDQ_ENG_MDP_RDMA1))) {
		module = "MDP";
	} else {
		if ((engineFlag & (1LL << CMDQ_ENG_MDP_RSZ0)) &&
		    (engineFlag &
		     (1LL << CMDQ_ENG_MDP_RSZ1))) { /* 1-in 2-out */
			...
			if ((state[0] == 0xa) && (state[1] == 0xa))
				module =
				    "ISP (caused mdp upstream hang)";
		  /* 1,0,1,0 */
		} else { /* 1-in 1-out */
			if (engineFlag & (1LL << CMDQ_ENG_MDP_RSZ0)) {
				CMDQ_REG_SET32(MDP_RSZ0_BASE + 0x040,
					       0x00000002);
				state[0] =
				    CMDQ_REG_GET32(MDP_RSZ0_BASE + 0x044) & 0xF;
			}
			if (engineFlag & (1LL << CMDQ_ENG_MDP_RSZ1)) {
				CMDQ_REG_SET32(MDP_RSZ1_BASE + 0x040,
					       0x00000002);
				state[1] =
				    CMDQ_REG_GET32(MDP_RSZ1_BASE + 0x044) & 0xF;
			}
			if ((state[0] == 0xa) || (state[1] == 0xa))
				module =
				    "ISP (caused mdp upstream hang)";
